...
const BRAND_LOGOS = {
  ...
  'diarco':     { domain: 'diarco.com.ar',     colors: ['#E8830C','#ffa726'], local: 'DIARCO Barrio.png' },
...
function getBrandInfo(name) {
  ...
  // Sort entries by length descending so longer brand names are matched first (e.g. 'diarco' before 'dia')
  const entries = Object.entries(BRAND_LOGOS).sort((a, b) => b[0].length - a[0].length);
...
function renderSummary() {
  ...
  const totalAhorro = list.reduce((s,d) => s + Math.max(0, (d.limitAmount||0) - (d.consumido||0)), 0);
  $('summaryBar').innerHTML = `
    <div class="summary-pill"><div class="num">${list.length}</div><div class="lbl">Disponibles</div></div>
